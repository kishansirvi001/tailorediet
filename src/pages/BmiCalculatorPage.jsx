...
function BmiCalculatorPage() {
  ...
  function updateWeight(event) {
    setForm((current) => ({ ...current, weight: unitSystem === 'metric' ? event.target.value : lbToKg(event.target.value).toFixed(2) }))
  }
  ...
  function updateHeightImperial(part, value) {
    ...
    setForm((current) => ({ ...current, height: feetInchesToCm(nextFeet, nextInches).toFixed(2) }))
  }

  const result = useMemo(() => calculateBmi(form), [form])
  const issues = useMemo(() => validateBodyMetrics({ age: 30, weight: form.weight, height: form.height }), [form])
    ...
    <SiteShell>
      <section className="mx-auto grid max-w-7xl gap-8 px-4 pb-14 pt-8 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-10 lg:pb-24 lg:pt-20">
        <div>
          ...
          <h1 className="mt-4 font-['Georgia'] text-4xl font-bold tracking-tight text-stone-950 sm:text-5xl md:text-7xl">BMI calculator</h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-stone-700">Give users a quick body mass index estimate in a dedicated, simpler flow.</p>
        ...
        <div className="rounded-[2rem] border border-stone-900/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(245,239,227,0.92))] p-5 shadow-sm sm:p-8">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl bg-stone-900 p-5 text-white"><p className="text-xs uppercase tracking-[0.16em] text-stone-400">BMI</p><p className="mt-2 text-3xl font-semibold">{result.bmi}</p></div>
            <div className="rounded-2xl bg-lime-200 p-5 text-stone-950"><p className="text-xs uppercase tracking-[0.16em] text-stone-700">Range</p><p className="mt-2 text-xl font-semibold">{result.label}</p></div>
            <div className="rounded-2xl border border-stone-900/10 bg-white p-5 text-stone-700"><p className="text-xs uppercase tracking-[0.16em] text-stone-500">Reference</p><p className="mt-2 text-sm leading-6">{result.note}</p></div>
          </div>
        </div>
      </section>
    </SiteShell>
