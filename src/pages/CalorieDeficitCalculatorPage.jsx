...
import { calculateCalorieDeficit, validateBodyMetrics } from '../lib/calculatorUtils.js'

function CalorieDeficitCalculatorPage() {
  ...
  const result = useMemo(() => calculateCalorieDeficit(form), [form])
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 pb-14 pt-8 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-10 lg:pb-24 lg:pt-20">
        <div>
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-stone-700 sm:text-lg md:leading-8">
            Estimate a daily calorie target for fat loss by subtracting a deliberate deficit from your maintenance intake.
          </p>
        ...
        <div className="rounded-[2rem] border border-stone-900/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(245,239,227,0.92))] p-5 shadow-sm sm:p-8">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="rounded-2xl border border-stone-900/10 bg-white/75 p-4 sm:col-span-2">
              <span className="text-xs uppercase tracking-[0.16em] text-stone-500">Deficit (kcal / day)</span>
              <input name="deficit" type="number" value={form.deficit} onChange={updateField} className="mt-2 w-full bg-transparent text-lg font-semibold text-stone-950 outline-none" />
            </label>
          </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="rounded-2xl bg-rose-200 p-5 text-stone-950">
              <p className="text-xs uppercase tracking-[0.16em] text-stone-700">Deficit</p>
              <p className="mt-2 text-3xl font-semibold">{result.deficit}</p>
            </div>
          ...
          <div className="mt-5 rounded-2xl border border-stone-900/10 bg-white/80 p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-stone-600">
              Keep protein high, recovery steady, and avoid making the deficit so aggressive that consistency breaks down.
            </p>
          </div>
        </div>
      </section>
...
export default CalorieDeficitCalculatorPage
